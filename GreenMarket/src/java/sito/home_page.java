...
public class home_page extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            authenticated = my_connect.authenticate(request.getParameter("name"), request.getParameter("password"));
            switch (authenticated) {
                case 0: {
                    HttpSession session = request.getSession(true);
                    session.setAttribute("user", request.getParameter("name"));
                    session.setAttribute("type_user", "buyer");
                ...
                case 1: {
                    HttpSession session = request.getSession(true);
                    session.setAttribute("user", request.getParameter("name"));
                    session.setAttribute("type_user", "seller");
                    response.sendRedirect("seller");
                }
                break;
                default:
                    response.sendRedirect("index.jsp?error=Utente o Password errati");
                    break;
            }
