            ...
            throws ServletException, IOException, Exception 
    {
        ...
        try 
        {
            ...
            if (type_user.equals("seller"))
            {
                ...
                out.println("<header><img src=\"img/user.png\" class=\"account\" /><img src=\"img/settings.png\" class=\"account\" /><p>Salve "+user+"</p><form method='post' action='logout'><input type='submit' id=\"logout\" value='Log-out' /></form></header>");            
                out.println("<div id=\"container_log\">");
                out.println("<form action='add_product' id='add' name='add' method='post' >");
                out.println("<table id='products' CELLSPACING='0' BORDER='0' >");
                out.println("<tr><th>Prodotto</th><th>Quantità</th><th>Prezzo*</th><th>Prezzo Totale</th><th>Fotografia</th></tr>");
                productsList = my_connect.get_product_by_user(user);   
                ...
                while(itr_list_products.hasNext()) {
                     i = i%2;
                     String rowClass = "";
                     if(i==0){rowClass="row1";}else{rowClass="row2";}
                     i++;
                    ...
                    if(exists(getURL+"/"+prodotto.photo)) {out.println("<td><img HEIGHT='80' src='"+ prodotto.photo +"' /></td>"); }
                    else{out.println("<td><img src='img/products/vuota.png' /></td>");}
                     out.println("</tr>");
                }
                out.println("<tr><td colspan='7' style='text-align:right'><input type='submit' value='Add' id='Compra' name='Aggiungi Prodotto' /></td></td></tr>");
                out.println("</table>");
                out.println("</form>");
                out.println("</div>");
                out.println("<footer>\n" +
